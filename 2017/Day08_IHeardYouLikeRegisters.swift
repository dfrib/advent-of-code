...
struct TheRegister {
    static var register: [String: Int] = [:]
    static var maxValue = 0

    static let predMapping: [String: (Int, Int) -> Bool] = [
        "<":  (<),  "<=": (<=),
        ">":  (>),  ">=": (>=),
        "==": (==), "!=": (!=)]

    static func eval(_ pred: [String]) -> Bool {
        return predMapping[pred[1]]!(register[pred[0]] ?? 0, Int(pred[2])!)
    }

    static let operMapping: [String: (inout Int, Int) -> ()] = [
        "inc": (+=),
        "dec": (-=)]

    static func operate(_ expr: [String]) {
        register[expr[0]] = register[expr[0]] ?? 0
        operMapping[expr[1]]!(&(register[expr[0]]!), Int(expr[2])!)
        maxValue = max(maxValue, register.values.max()!)
    }
}

let data = puzzleInput.components(separatedBy: "\n")
    .map { $0.components(separatedBy: "if").reversed().map { $0.split(separator: " ").map(String.init) } }

for instruction in data where TheRegister.eval(instruction[0]) {
    TheRegister.operate(instruction[1])
}

print("Part 1 =", TheRegister.register.values.max()!)
print("Part 2 =", TheRegister.maxValue)
