...
extension String {
    ...
    // Brute-force parse it.
    func nameAndWeight() -> (name: String, weight: Int) {
        let strs = split(separator: " ")
        let name = String(strs[0])
        let weight = Int(String(strs[1].dropFirst().dropLast())) ?? -1
        return (name, weight)
    }
}
...
class Program {
    ...
    // Brutely re-parse the child weights dict on each call for convenience.
    var childWeights: [Int: Int] {
        return children.reduce(into: [:]) { $0[$1.totalWeight] = ($0[$1.totalWeight] ?? 0) + 1 }
    }
    ...
    func addChild(_ childProgram: Program) {
        childProgram.parent = self
        children.append(childProgram)
        totalWeight += childProgram.totalWeight
        var program = self
        while let parent = program.parent {
            parent.totalWeight += childProgram.totalWeight
            program = parent
        }
    }
}
...
// Find unbalanced program upstream (updisk) from a provided program.
// Assumes exactly a single program has an invalid weight.
func findUnbalancedProgram(from program: Program, _ inbalanceType: Inbalance?) -> (Program, Inbalance?) {
    var inbalance = inbalanceType

    // If just a single child, then progress directly
    // to it (thid child could have an unbalanced disk).
    if program.children.count == 1 {
        return findUnbalancedProgram(from: program.children.first!, inbalance)
    }
    // Unbalanced due to unbalanced disk.
    else if program.childWeights.keys.count > 1 {

        // For first unbalanced disk (starting at root), look at children
        // to decide the direction of the inbalance (unless only two children;
        // that wont help us).
        if inbalance == nil && program.children.count != 2 {
            let weights = program.childWeights.sorted { $0.value < $1.value }.map { $0.key }
            let outlierInlierDiff = weights[0] - weights[1]
            inbalance = outlierInlierDiff > 0 ? .tooLarge(outlierInlierDiff) : .tooSmall(-outlierInlierDiff)
        }
        // Special case: yet to find inbalance but exactly two children on
        // and inbalanced disk: visit both.
        else if program.children.count == 2 {
            for child in program.children {
                return findUnbalancedProgram(from: child, inbalance)
            }
        }
        // If we've identified the type of inbalance, then visit
        // the specific child that contains the inbalance.
        if let inbalance = inbalance {
            switch(inbalance) {
            case .tooSmall: return findUnbalancedProgram(from: program.children.min { $0.totalWeight < $1.totalWeight }!, inbalance)
            case .tooLarge: return findUnbalancedProgram(from: program.children.max { $0.totalWeight < $1.totalWeight }!, inbalance)
            }
        }
    }

    // Found the unbalanced program.
    return (program, inbalance)
}

// ------------------------------------------------------------------ //

// First raw parsing of input.
var rawInput = puzzleInput
    .split(separator: "\n")
    .map { $0.split(separator: ">").map(String.init) }

// Parse programs.
var programs = rawInput.flatMap { $0.first }
    .reduce(into: [String: Program]()) { programs, rawProgram in
        let (name, weight) = rawProgram.nameAndWeight()
        programs[name] = Program(name, weight)
}

// Parse program descendants/children (also populating total weights).
for (rawParent, children) in rawInput
    .flatMap({ $0.count > 1 ? ($0[0], $0[1]) : nil }) {
        let (parent, _) = rawParent.nameAndWeight()
        for program in children.split(separator: ",")
            .flatMap({ $0.split(separator: " ").filter { !$0.isEmpty }.map(String.init) })
            .flatMap({ programs[$0] }){
                programs[parent]?.addChild(program)
        }
}

// Pick a random program and progress down to root.
var rootProgram = programs.values.first!
while let parent = rootProgram.parent { rootProgram = parent }

// Go for it!
let (unbalancedProgram, inbalance) = findUnbalancedProgram(from: rootProgram, nil)

// Correct the faulty weighted program
var correctWeight = unbalancedProgram.weight
if case .some(.tooLarge(let diff)) = inbalance { correctWeight -= diff }
else if case .some(.tooSmall(let diff)) = inbalance { correctWeight += diff }

// Results.
print("Part 1 =", rootProgram.name)
print("Part 2 =", correctWeight)
